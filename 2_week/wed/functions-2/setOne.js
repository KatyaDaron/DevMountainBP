...
// CODE HERE
const myFunc = (parameter1, parameter2) => console.log(`The first parameter is ${parameter1}. The second parameter is ${parameter2}`);
...
greeting(`Katya`, `Daron`, (fullName) => console.log(`Hello, my full name is ${fullName}`));
...
function pricesPlusTax(prices, cb){
    for(let i = 0; i < prices.length; i++) {
        totalCost.push(prices[i] + prices[i] * .2)
    }
    cb(totalCost);
}
